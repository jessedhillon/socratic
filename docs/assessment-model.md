...
| `proficiency_descriptions` | What each grade level looks like (see below)      |
| `weight`                   | Relative importance in overall grade              |

#### Proficiency Description Model

Socratic uses a **proficiency description model** for rubrics rather than an evidence indicator checklist model. This design decision is driven by the requirements of LLM-based grading.

**Two Models Considered:**

1. **Evidence Indicator Model**: Lists discrete skills/behaviors to check for. Grading logic: count how many indicators are present, map to a grade.

2. **Proficiency Description Model**: Describes what each grade level (S/A/C/F) looks like qualitatively, including failure modes. Grading logic: compare response to level descriptions, classify.

**Why Proficiency Descriptions are Superior for LLM Grading:**

1. **LLMs excel at holistic comparison** — "Does this response resemble mastery or partial mastery?" is a natural classification task for language models. They can assess the overall quality of understanding rather than mechanically checking boxes.

2. **Failure modes are critical** — A response might technically hit all evidence indicators while fundamentally missing the point. For example, in literary analysis, a student could produce "polished literary language with no defensible interpretation" — this would pass indicator checks but represents a failure mode. Proficiency descriptions can explicitly capture these anti-patterns.

3. **Avoids the checklist problem** — With indicators, sophisticated-sounding responses can "check all boxes" while lacking genuine understanding. This is especially problematic when LLMs might help students craft responses. Proficiency descriptions capture qualitative differences between someone who truly understands versus someone performing understanding.

4. **Direct mapping to grades** — No translation step from "3 of 4 indicators present" to a letter grade. The LLM directly classifies against grade-level descriptions.

5. **Captures the "adjacent failure"** — Partial mastery isn't just "fewer indicators" — it's a qualitatively different type of understanding. Someone at partial mastery might have correct facts but wrong framing, or understand the concept but not its implications.

**Proficiency Level Structure:**

Each criterion should describe what responses look like at each level:

| Level           | Grade | Description Focus                                        |
| --------------- | ----- | -------------------------------------------------------- |
| Mastery         | S     | What genuine understanding looks like                    |
| Partial Mastery | A     | Common gaps or limitations in otherwise sound responses  |
| Developing      | C     | Partial understanding with significant misconceptions    |
| Insufficient    | F     | Fundamental misunderstanding or inability to demonstrate |
| Red Flag        | —     | Sophisticated-seeming responses that are actually hollow |

The "Red Flag" category is not a grade but a detection pattern — responses matching red flags should be scrutinized carefully as they may represent parroted or LLM-generated content that lacks genuine understanding.
