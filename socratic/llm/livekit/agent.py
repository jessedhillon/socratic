...
from socratic.llm.agent.assessment.state import CriterionCoverage
...
class SocraticAssessmentAgent(Agent):  # pyright: ignore [reportUntypedBaseClass]
    ...
    def _build_initial_state(self) -> dict[str, t.Any]:
        ...
        try:
            async for event in self.graph.astream_events(
                ...
                if event["event"] != "on_chat_model_stream":
                    continue
                token = event["data"]["chunk"].content  # pyright: ignore [reportTypedDictNotRequiredAccess]
                if not token:
                    continue
                full_response += token
    ...
    async def llm_node(  # pyright: ignore [reportIncompatibleMethodOverride]
        ...
        try:
            ...
            logger.error(f"Assessment error for attempt {self.attempt_id}: {exc}", exc_info=True)
